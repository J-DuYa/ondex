...
/**
 * Showing table for view/edit of mapping DataSource to colour.
 * 
 * @author taubertj
 * @version 10.07.2008
 */
public class DialogDataSourceColor extends OVTK2Dialog {

	// generated
	private static final long serialVersionUID = 8911526372131237672L;

	private static final String COLOR_VIS = "DataSource.Color.";

	// model containing mapping DataSource to colour mapping
	...
	/**
	 * Constructs user input to view/edit mapping DataSource to colour.
	 * 
	 * @param selection
	 *            pre-selection in table
	 */
	public DialogDataSourceColor(String selection) {
		super("Dialog.CvColor.Title", "Properties16.gif");
		this.setDefaultCloseOperation(HIDE_ON_CLOSE);
		this.getContentPane().setLayout(new BorderLayout());
		this.getContentPane().add(makeProperties(selection),
				BorderLayout.CENTER);
		this.getContentPane().add(
				makeButtonsPanel("Dialog.CvColor.Apply",
						"Dialog.CvColor.Cancel"), BorderLayout.SOUTH);
		this.pack();
	}
	 ...
	 * 
	 */
	public DialogDataSourceColor() {
		this(null);
	}

	/**
	 * Creates the properties panel for mapping DataSource to colour.
	 * 
	 * @param selection
	 *            what is highlighted
	 * @return JPanel
	...
	private JPanel makeProperties(String selection) {
		...
		model = new ColorTableModel(colors, Config.language
				.getProperty("Dialog.CvColor.TableName"), Config.language
				.getProperty("Dialog.CvColor.TableColor"));
		...
		int width = Config.language.getProperty("Dialog.CvColor.TableColor")
				.length() * 7;
	...
	public int setSelection(String selection) {
		...
		if (selection != null) {
			...
			// if DataSource not found, insert it
			else {
	...
	/**
	 * Performs saving of colour associated with DataSource
	 */
