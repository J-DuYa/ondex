...
/**
 * Passes XML encoding of a list of RelationTypes to output stream or simply
 * HTML.
 * 
 * @author taubertj
 * 
...
public class RelationTypesMessageBodyWriter extends Export implements
		MessageBodyWriter<Set<RelationType>> {

	public RelationTypesMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(Set<RelationType> iterator, Class<?> clazz, Type type,
	...
	@Override
	public void writeTo(Set<RelationType> iterator, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export list of RelationTypes
				buildRelationTypes(xmlWriteStream, iterator);
		...
		// return HTML encoding
			...
			writer.write("<h2>RelationTypes</h2>\n");
			writer.write("<table><tr><th>id</th><th>fullname</th></tr>\n");
			for (RelationType relationtype : iterator) {
				writer.write("<tr><td><a href=\"");
				writer.write(path + "/" + relationtype.getId());
				writer.write("\">");
				writer.write(relationtype.getId());
				writer.write("</a>");
				writer.write("</td><td>");
