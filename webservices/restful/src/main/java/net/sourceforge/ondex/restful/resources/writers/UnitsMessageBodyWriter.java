...
/**
 * Passes XML encoding of a list of units to output stream or simply HTML.
 * 
 * @author taubertj
 * 
 */
...
public class UnitsMessageBodyWriter extends Export implements
		MessageBodyWriter<Set<Unit>> {

	public UnitsMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(Set<Unit> iterator, Class<?> clazz, Type type,
	...
	public boolean isWriteable(Class<?> clazz, Type type, Annotation[] anno,
		...
		if (type instanceof ParameterizedType) {
			...
			if (first instanceof Class<?>)
				accept = accept
						&& Unit.class.isAssignableFrom((Class<?>) first);
			else
				accept = false;
		} else
			accept = false;
	...
	@Override
	public void writeTo(Set<Unit> iterator, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export list of units
				buildUnits(xmlWriteStream, iterator);
