...
/**
 * Passes XML encoding of a list of ConceptNames to output stream or simply
 * HTML.
 * 
 * @author taubertj
 * 
...
public class ConceptNamesMessageBodyWriter extends Export implements
		MessageBodyWriter<Set<ConceptName>> {

	public ConceptNamesMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(Set<ConceptName> iterator, Class<?> clazz, Type type,
	...
	public boolean isWriteable(Class<?> clazz, Type type, Annotation[] anno,
		...
		if (type instanceof ParameterizedType) {
			...
			if (first instanceof Class<?>)
				accept = accept
						&& ConceptName.class.isAssignableFrom((Class<?>) first);
			else
				accept = false;
		} else
			accept = false;
	...
	@Override
	public void writeTo(Set<ConceptName> iterator, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export list of ConceptNames
				buildConceptNames(xmlWriteStream, iterator);
		...
		// return HTML encoding
			...
			writer.write("<h2>ConceptNames</h2>\n");
			writer.write("<table><tr><th>name</th><th>is preferred</th></tr>\n");
			for (ConceptName conceptname : iterator) {
				writer.write("<tr><td><a href=\"");
				writer.write(path + "/" + conceptname.getName());
				writer.write("\">");
				writer.write(conceptname.getName());
				writer.write("</a>");
				writer.write("</td><td>");
				writer.write(conceptname.isPreferred() + "</td></tr>\n");
			}
			writer.write("</table>\n");
			writer.write("<a href=\"");
