...
/**
 * Passes XML encoding of a list of AttributeNames to output stream or simply
 * HTML.
 * 
 * @author taubertj
 * 
...
public class AttributeNamesMessageBodyWriter extends Export implements
		MessageBodyWriter<Set<AttributeName>> {

	public AttributeNamesMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(Set<AttributeName> iterator, Class<?> clazz, Type type,
	...
	@Override
	public void writeTo(Set<AttributeName> iterator, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export list of AttributeNames
				buildAttributeNames(xmlWriteStream, iterator);
		...
		// return HTML encoding
			...
			writer.write("<h2>AttributeNames</h2>\n");
			writer.write("<table>\n");
			writer.write("<tr><th>id</th><th>fullname</th></tr>\n");
			for (AttributeName attributename : iterator) {
				writer.write("<tr><td><a href=\"");
				writer.write(path + "/" + attributename.getId());
				writer.write("\">");
				writer.write(attributename.getId());
				writer.write("</a>");
				writer.write("</td><td>");
