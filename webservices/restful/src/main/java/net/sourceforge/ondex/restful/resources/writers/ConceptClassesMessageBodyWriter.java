...
/**
 * Passes XML encoding of a list of ConceptClasses to output stream or simply
 * HTML.
 * 
 * @author taubertj
 * 
...
public class ConceptClassesMessageBodyWriter extends Export implements
		MessageBodyWriter<Set<ConceptClass>> {

	public ConceptClassesMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(Set<ConceptClass> iterator, Class<?> clazz, Type type,
	...
	@Override
	public void writeTo(Set<ConceptClass> iterator, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export list of ConceptClasses
				buildConceptClasses(xmlWriteStream, iterator);
		...
		// return HTML encoding
			...
			writer.write("<h2>ConceptClasses</h2>\n");
			writer.write("<table><tr><th>id</th><th>fullname</th></tr>\n");
			for (ConceptClass conceptclass : iterator) {
				writer.write("<tr><td><a href=\"");
				writer.write(path + "/" + conceptclass.getId());
				writer.write("\">");
