...
/**
 * Passes XML encoding of a DataSource to output stream or simply HTML.
 * 
 * @author taubertj
 * 
 */
...
public class DataSourceMessageBodyWriter extends Export implements
		MessageBodyWriter<DataSource> {

	public DataSourceMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(DataSource dataSource, Class<?> clazz, Type type,
	...
	public boolean isWriteable(Class<?> clazz, Type type, Annotation[] anno,
			MediaType mediatype) {
		// accept all subclasses of DataSource
		return DataSource.class.isAssignableFrom(clazz);
	}

	@Override
	public void writeTo(DataSource dataSource, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
			MultivaluedMap<String, Object> map, OutputStream outstream)
			throws IOException, WebApplicationException {

		// make sure there is a DataSource
		if (dataSource == null)
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export DataSource
				buildDataSource(xmlWriteStream, dataSource);
