...
/**
 * Passes XML encoding of a ConceptName to output stream or simply HTML.
 * 
 * @author taubertj
 * 
 */
...
public class ConceptNameMessageBodyWriter extends Export implements
		MessageBodyWriter<ConceptName> {

	public ConceptNameMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(ConceptName conceptname, Class<?> clazz, Type type,
	...
	public boolean isWriteable(Class<?> clazz, Type type, Annotation[] anno,
			MediaType mediatype) {
		// accept all subclasses of ConceptName
		return ConceptName.class.isAssignableFrom(clazz);
	}

	@Override
	public void writeTo(ConceptName conceptname, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
			MultivaluedMap<String, Object> map, OutputStream outstream)
			throws IOException, WebApplicationException {

		// make sure there is a ConceptName
		if (conceptname == null)
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export ConceptName
				buildConceptName(xmlWriteStream, conceptname);
		...
		// return HTML encoding
			...
			writer.write("<h2>ConceptName</h2>\n");
			writer.write("<table>\n");
			writer.write("<tr><td>name</td><td>");
			writer.write(conceptname.getName());
			writer.write("</td></tr>\n");
			writer.write("<tr><td>is preferred</td><td>");
