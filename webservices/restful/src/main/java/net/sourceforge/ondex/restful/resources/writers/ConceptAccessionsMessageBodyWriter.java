...
/**
 * Passes XML encoding of a list of ConceptAccessions to output stream or simply
 * HTML.
 * 
 * @author taubertj
 * 
...
public class ConceptAccessionsMessageBodyWriter extends Export implements
		MessageBodyWriter<Set<ConceptAccession>> {

	public ConceptAccessionsMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(Set<ConceptAccession> iterator, Class<?> clazz,
	...
	@Override
	public void writeTo(Set<ConceptAccession> iterator, Class<?> clazz,
			Type type, Annotation[] anno, MediaType mediatype,
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export list of ConceptAccessions
				buildConceptAccessions(xmlWriteStream, iterator);
		...
		// return HTML encoding
			...
			writer.write("<h2>ConceptAccessions</h2>\n");
			writer.write("<table><tr><th>accession</th><th>element of</th></tr>\n");
			for (ConceptAccession conceptaccession : iterator) {
				writer.write("<tr><td><a href=\"");
				writer.write(path + "/" + conceptaccession.getAccession() + "/"
