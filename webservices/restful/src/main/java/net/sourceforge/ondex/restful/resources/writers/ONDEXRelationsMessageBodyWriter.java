...
/**
 * Passes XML encoding of a list of relations to output stream or simply HTML.
 * 
 * @author taubertj
 * 
 */
...
public class ONDEXRelationsMessageBodyWriter extends Export implements
		MessageBodyWriter<Set<ONDEXRelation>> {

	public ONDEXRelationsMessageBodyWriter() throws JAXBException {
		super();
	}
	...
	@Override
	public long getSize(Set<ONDEXRelation> view, Class<?> clazz, Type type,
	...
	@Override
	public void writeTo(Set<ONDEXRelation> view, Class<?> clazz, Type type,
			Annotation[] anno, MediaType mediatype,
		...
		if (mediatype.toString().equals(MediaType.APPLICATION_XML)) {
			try {
				...
				// export list of relations
				buildRelations(xmlWriteStream, view);
		...
		// return HTML encoding
			...
			if (!path.endsWith("relations"))
				path = path.substring(0, path.indexOf("relations") + 9);

			// simply write HTML code
			OutputStreamWriter writer = new OutputStreamWriter(outstream);
			writer.write("<h2>Relations</h2>\n");
			writer.write("<table>\n");
			writer.write("<tr><th>id</th><th>from</th><th>to</th></tr>\n");
			int count = 0;
			for (ONDEXRelation relation : view) {
				...
				writer.write(path + "/" + String.valueOf(relation.getId()));
				writer.write("\">");
				writer.write(String.valueOf(relation.getId()));
				writer.write("</a>");
				writer.write("</td><td>");
				writer.write(String.valueOf(relation.getFromConcept().getId()));
				writer.write("</td><td>");
				writer.write(String.valueOf(relation.getToConcept().getId()));
				writer.write("</td></tr>\n");
				count++;
			}
			writer.write("</table>\n");
