...
/**
 * Returns a concept for a plain id in path.
 * 
 * @author taubertj
 * 
 */
@Provider
public class ONDEXConceptInjectableProvider implements
	 ...
	 *            HttpContext
	 */
	public ONDEXConceptInjectableProvider(@Context HttpContext hc) {
		this.hc = hc;
	}
	...
	public Injectable<?> getInjectable(ComponentContext cc, PathParam pp,
		...
		// only accepts parameters dealing with ONDEXConcept
		if (!ONDEXConcept.class.isAssignableFrom(param.getParameterClass()))
			return null;

		final String name = param.getSourceName();
		return new Injectable<ONDEXConcept>() {
			public ONDEXConcept getValue() {
				...
				// return concept for id
				String value = hc.getUriInfo().getPathParameters()
						.getFirst(name);
				return graph.getConcept(Integer.valueOf(value));
			}
		};
	}
