...
public class PluginWS {
    ...
    protected static WebServiceEngine webServiceEngine =
    ...
    public static void addDefaultArgements(ONDEXPlugin plugin, ONDEXPluginArguments arguments)
        ...
        try {
            for (ArgumentDefinition<?> aDefinition : argumentDefinitions) {
                ...
                if (arguments.getObjectValueList(aDefinition.getName()).size() > 0) {
                    ...
                    logger.info("No value provided for " + aDefinition.getName() +
                            " using default value of: " + aDefinition.getDefaultValue());
                    createArguement(plugin, arguments, aDefinition.getName(), aDefinition.getDefaultValue());
                }
                else if (aDefinition.isRequiredArgument()) {
                    throw new IllegalArguementsException("No value found and no default available" +
                            " for arguement with the name: " + aDefinition.getName() +
                            " in the plugin " + plugin.getClass(), plugin, logger);
                }
                else {
            ...
            throw new IllegalArguementsException("Unexpected mismatch between annouced deafults " +
                    "and excepted aurguements", plugin, logger);
        }
        //return true;
    }

   protected static void checkArguementName (ONDEXPlugin plugin, String arguementName)
           throws IllegalArguementsException {
        ...
        throw new IllegalArguementsException("Plugin has been changed!  "+
                "Arguement with the name: " + arguementName +
                " no longer in the plugin " + plugin.getClass(), plugin, logger);
    }

    protected static void createArguement(ONDEXPlugin plugin, ONDEXPluginArguments arguments,
                                       String arguementName, Object arguementValue)
            throws IllegalArguementsException {
        ...
        try {
            ...
            throw new IllegalArguementsException("Unable to add value " + arguementValue +
                    " for arguement with the name: " + arguementName +
                    " in the plugin " + plugin.getClass() +
                    " exception thrown was " , e, plugin, logger);
        }
    }

    protected void createArguement(ONDEXPlugin plugin, ONDEXPluginArguments arguments,
                                   String arguementName, Object[] arguementValues)
            throws IllegalArguementsException {
        checkArguementName (plugin, arguementName);
        ...
        try {
            ...
            throw new IllegalArguementsException("Unable to add values " + arguementValues +
                    " for arguement with the name: " + arguementName +
                    " in the plugin " + plugin.getClass() +
                    " exception thrown was " , e, plugin, logger);
        }
    ...
    protected void addExportFile(ONDEXPlugin plugin, ONDEXPluginArguments arguments, File file)
            throws IllegalArguementsException {
    ...
    protected void addExportFile(ONDEXPlugin plugin, String aurguementName, ONDEXPluginArguments arguments, File file)
            throws IllegalArguementsException {
   ...
   private static void checkExists(File dir) {
        ...
        //logger.info("checking: "+parent.getAbsolutePath());
        if (dir.exists()) {
            logger.info("exists");
            return;
        }
     ...
     */
    public static InputStream ZipToInputStream(String argument)
            throws InvalidPluginArgumentException{
     ...
     * @throws InvalidPluginArgumentException
     */
    public static InputStream StringToPureInputStream(String argument)
            throws InvalidPluginArgumentException{
        ...
        if (file.exists()){
            ...
            try {
                ...
                throw new InvalidPluginArgumentException("Exception attempting to convert existing file "
                        + argument + " to an InputStream."+ex);
            }
        } else {
            try {
                ...
                throw new InvalidPluginArgumentException("The arguement " + argument +
                        " of type String, could not be converted "+
     ...
     * @throws InvalidPluginArgumentException
     */
    public static InputStream GZipToInputStream(String argument)
            throws InvalidPluginArgumentException{
        InputStream pureStream = StringToPureInputStream(argument);
        try {
            return new GZIPInputStream(pureStream);
        } catch (IOException ex) {
            throw new InvalidPluginArgumentException("Exception attempting to convert InputStream  "
                    + argument + " to a GZIPInputStream."+ex);
        }
     }

    protected static void createInputFileArguement(ONDEXPlugin plugin, ONDEXPluginArguments arguments,
            String arguementName, String asString, byte[] asArray) throws CaughtException{
        try{
            File inputFile;
    ...
    protected void createOptionalInputFileArguement(ONDEXPlugin plugin, ONDEXPluginArguments arguments,
            String arguementName, String asString, byte[] asArray) throws CaughtException{
         if (asString == null || asString.isEmpty()){
    ...
    private static final void toZip (ZipOutputStream out, File file, String path) throws IOException {
        //ystem.out.println("Adding: "+path+file.getName());
        if (file.isFile()){
            //ystem.out.println("Adding File: "+path+file.getName());
   ...
   protected File zipFile(File file, ZipFormat zipFormat) throws IOException{
        switch (zipFormat){
            ...
            case TAR: return tar(file);
            case TAR_GZIP: return tarGzip(file);
            case GZIP: return gzip(file);
            case URL: 
            case RAW: 
                if (file.isFile()){
                    return file;
    ...
    public static String initMetaData(ONDEXGraph graph) throws CaughtException {
        try {
            ...
            //Initialisation not used as it depends on the plugin registry
            net.sourceforge.ondex.parser.oxl.Parser parser = new net.sourceforge.ondex.parser.oxl.Parser();
            //set the inputFile arguement.
            String md = System.getProperty("webapp.root") + File.separator + "WEB-INF" + File.separator + "classes"
                    + File.separator + "data" + File.separator + "xml" + File.separator + "ondex_metadata.xml";
            ...
            //Run the oxl parser which sets the other arguements to the defaults.
    ...
    protected String runExport (ONDEXGraph graph, File file, ONDEXExport export,
    ...
    protected static String runOXLParser (ONDEXGraph graph,
           ONDEXParser parser, ONDEXPluginArguments args)
    ...
    protected String runParser (ONDEXGraph graph,
           ONDEXParser parser, ONDEXPluginArguments args)
           throws WebserviceException {
      BufferedOndexListener bufferedOndexListener
                 = new BufferedOndexListener(logger);
    ...
    protected String runParser (ONDEXGraph graph, File file, ONDEXParser parser, ONDEXPluginArguments args,
            ZipFormat zipFormat) throws WebserviceException {
        BufferedOndexListener bufferedOndexListener
                 = new BufferedOndexListener(logger);
        try {
            parser.addONDEXListener(bufferedOndexListener);
