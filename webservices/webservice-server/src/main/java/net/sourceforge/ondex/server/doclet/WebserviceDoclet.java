...
public class WebserviceDoclet {
	...
	public static boolean start(RootDoc root) {
		try {
            ...
            writeDoc("exportUsingJob.html", "net.sourceforge.ondex.server.plugins.export.ExportUsingJobAuto",
                    "Export", root);
			writeDoc("filter.html", "net.sourceforge.ondex.server.plugins.filter.FilterAuto", "Filter", root);
			writeDoc("filterUsingJob.html", "net.sourceforge.ondex.server.plugins.filter.FilterUsingJobAuto", "Filter",
                    root);
			writeDoc("parser.html", "net.sourceforge.ondex.server.plugins.parser.ParserAuto", "Parser", root);
			writeDoc("parserUsingJob.html", "net.sourceforge.ondex.server.plugins.parser.ParserUsingJobAuto",
                    "Parser", root);
            writeDoc("mapping.html", "net.sourceforge.ondex.server.plugins.mapping.MappingAuto", "Mapping", root);
            writeDoc("mappingUsingJob.html", "net.sourceforge.ondex.server.plugins.mapping.MappingUsingJobAuto",
                    "Mapping", root);
            writeDoc("transformer.html", "net.sourceforge.ondex.server.plugins.transformer.TransformerAuto",
                     "Transformer", root);
            writeDoc("transformerUsingJob.html",
                    "net.sourceforge.ondex.server.plugins.transformer.TransformerUsingJobAuto", "Transformer", root);
    ...
    private static void writeDoc(RootDoc root, PrintWriter out,
			ClassDoc classDoc, String pluginName) {
	 ...
	 * @param methods
	 */
	private static void writeOperationsTable(PrintWriter out,
			MethodDoc[] methods) {
            ...
            if (tag.kind().equals("@see")) {
                ...
                if (seeTag.referencedClass() == null) {
                    //if (seeTag.referencedClassName() != null){
                        //ystem.out.println(seeTag.referencedClassName());
                    //    ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
                    //    try {
                    //        Class theClass = classLoader.loadClass(seeTag.referencedClassName());
                            //ystem.out.println(theClass);
                    //    } catch (ClassNotFoundException ex) {
                    //        Logger.getLogger(WebserviceDoclet.class.getName()).log(Level.SEVERE, null, ex);
                    //    }

                    //} else {
                    System.err.println("seeTag: "+seeTag +
                            " referencedClass not found. " +
                            "Is it correct and listed in the imports");
                    success = false;
    		    out.print(tag.text());
                    //}
                    ...
                    if (seeTag.referencedMember().isField()){
                        //ystem.out.println("SeeTag: "+tag);
                        //String className = seeTag.referencedClassName();
                        //ystem.out.println("className "+className);
                        //ystem.out.println("ref name:"+fieldName);
                        ClassDoc classDoc = seeTag.referencedClass();
                        //ystem.out.println("CalssDoc: "+classDoc);
                        FieldDoc[] fieldDocs = classDoc.fields();
                        //ystem.out.println(fieldDocs.length);
                        for (FieldDoc fieldDoc: fieldDocs){
                            //ystem.out.println(fieldDoc);
                            ...
                            //ystem.out.println("\""+fieldDoc.name()+"\"!=\""+fieldName+"\"");
                        }
                        System.err.println("seeTag: "+seeTag +
                                " referencedMember " + fieldName +
                    ...
                    //ystem.out.println("getTypeName(seeTag.referencedClass())");
                    //ystem.out.println(getTypeName(seeTag.referencedClass()));
	 ...
	 * @param typeClasses
	 */
	private static void writeTypesTable(PrintWriter out,
			List<ClassDoc> typeClasses) {
    ...
    private static boolean webMethodExcludeFalse (MethodDoc method){
        ...
        if (!method.isPublic()){
            //Not interested in non public methods!
        ...
        if (excludeFalse && !webResult){
            System.err.println("*** Method "+method+
                    " does not have an @WebResult tag");
            success = false;
        }
        if (!excludeFound){
            System.err.println("*** Method "+method+
                    " does not have an @WebMethod tag");
            success = false;
        }
        return excludeFalse;
   }

	private static void writeOperationDetails(PrintWriter out,
			MethodDoc[] methods) {
	 ...
	 * @param typeClasses
	 */
	private static void writeTypeDetails(PrintWriter out,
			List<ClassDoc> typeClasses) {
	...
	private static String getTypeName(Type type) {
		...
		if (typeName.startsWith("WS")) {
            ...
            //ystem.out.println("get type called with "+type);
	...
	private static void writeParametersTable(PrintWriter out, MethodDoc method) {
		...
		for (Parameter parameter : method.parameters()) {
            ...
            for (AnnotationDesc annotation : annotations){
                ...
                ElementValuePair[] pairs =  annotation.elementValues();
                for (ElementValuePair pair : pairs){
                    //ystem.out.println(pair.value().value());
            ...
            //ystem.out.println("parameterTag= "+parameterTag);
            //ystem.out.println(parameter);
            //ystem.out.println(parameter.name());
			out.println("<tr class=\"" + (odd ? "odd" : "even") + "\">");
			out.print("<td>");
			out.print(name);
			out.println("</td>");
            ...
            if (parameterTag != null){
                //if (parameterTag.inlineTags().length>0){
                    writeTags(out, parameterTag.inlineTags());
                //} else {
                    //ystem.out.println("Empty inLineTags in "+method+" for "+parameter.name());
                //}
            } else {
	 ...
	 * @return
	 */
	private static ParamTag findParameterTag(MethodDoc method,
			Parameter parameter) {
        //ystem.out.println( method+ " "+ parameter);
        //ystem.out.println( method.name());
        //ystem.out.println("length ="+ method.paramTags().length);

		for (ParamTag parameterTag : method.paramTags()) {
            String pName = parameterTag.parameterName();
            if (pName.startsWith("@see")){
                //ystem.out.println("Testing "+pName);
                pName = pName.substring(4,pName.length());
                //ystem.out.println("Using pName"+pName);
            }
	...
	private static void writeHeader(PrintWriter out, String classType) {
		...
		//if (classType.endsWith("Filter")) {
		//	type = "filter";
		//} else if (classType.endsWith("Parser")) {
		//	type = "parser";
		//}
        ...
        out.println("<br>");
		out.println("<p>WSDL: <a href=\"services/"+serviceName+
                "?wsdl\">services/"+serviceName+"?wsdl</a></p>");
	}
