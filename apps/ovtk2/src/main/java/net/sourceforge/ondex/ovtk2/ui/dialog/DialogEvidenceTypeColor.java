...
public class DialogEvidenceTypeColor extends OVTK2Dialog {
	...
	public DialogEvidenceTypeColor(String selection) {
		...
		this.getContentPane().add(
				makeButtonsPanel("Dialog.EtColor.Apply",
						"Dialog.EtColor.Cancel"), BorderLayout.SOUTH);
		this.pack();
	}
	...
	private JPanel makeProperties(String selection) {
		...
		model = new ColorTableModel(colors, Config.language
				.getProperty("Dialog.EtColor.TableName"), Config.language
				.getProperty("Dialog.EtColor.TableColor"));
		...
		int width = Config.language.getProperty("Dialog.EtColor.TableColor")
				.length() * 7;
