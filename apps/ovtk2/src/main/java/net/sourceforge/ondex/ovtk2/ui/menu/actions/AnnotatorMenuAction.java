 ...
 * @author taubertj
 */
public class AnnotatorMenuAction implements ActionListener,
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (cmd.startsWith("Menu.Analysis.")) {
            ...
            Constructor<?> constr;
            try {
                constr = Class.forName(className).getConstructor(
                        new Class<?>[]{OVTK2Viewer.class});

                JInternalFrame frame = (JInternalFrame) constr
                        .newInstance(viewer);
                    ...
                    public void task() {
                        try {
                            ...
                            pack = pack.substring(pack.lastIndexOf(".") + 1,
                                    pack.length());
                            PluginID plid = new PluginID(pack, className
                            ...
                            JInternalFrame annotatorFrame = new RegisteredJInternalFrame(
                                    annotator_new.getName(), "Annotator",
                                    annotator_new.getName() + " - "
                                            + viewer.getTitle(), true, true,
                                    false, true);
                            annotatorFrame
                                    .addInternalFrameListener(new OVTK2AnnotatorInternalFrameListener(
                                            annotator_new));
                            ...
                            annotatorFrame
                                    .addInternalFrameListener(new AnnotatorMenuAction());
                            if (!annotators.containsKey(viewer))
                                annotators.put(viewer,
    ...
    public void internalFrameClosing(InternalFrameEvent e) {
        if (e.getInternalFrame().getContentPane() instanceof OVTK2Annotator) {
            ...
            while (it.hasNext()) {
                ...
                Iterator<JInternalFrame> it2 = annotators.get(it.next())
                        .iterator();
                while (it2.hasNext()) {
            ...
            if (annotators.containsKey(e.getInternalFrame())) {
                for (JInternalFrame frame : annotators
                        .get(e.getInternalFrame()).toArray(
                                new JInternalFrame[annotators.size()])) {
                    close(frame);
                }
