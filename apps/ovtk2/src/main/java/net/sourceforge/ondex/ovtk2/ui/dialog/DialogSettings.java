...
public class DialogSettings extends OVTK2Dialog {
	...
	public DialogSettings() {
		...
		this.getContentPane().add(
				makeButtonsPanel("Dialog.Settings.Apply",
						"Dialog.Settings.Cancel"), BorderLayout.SOUTH);
		this.pack();
	}
	...
	private JPanel makeLoginPanel() {
		...
		if (Config.config.getProperty("Webservice.Enable") != null) {
			webserviceEnable = Boolean.valueOf(Config.config
					.getProperty("Webservice.Enable"));
		}
		...
		loginBorder = BorderFactory.createTitledBorder(Config.language
				.getProperty("Dialog.Settings.Login"));
		login.setBorder(loginBorder);

		// panel for user name input
		JLabel userLabel = new JLabel(
				Config.language.getProperty("Dialog.Settings.User"));
		...
		JLabel passwordLabel = new JLabel(
				Config.language.getProperty("Dialog.Settings.Password"));
		password = new JTextField(Config.config.getProperty("Program.Password"));
		password.setPreferredSize(new Dimension(fieldWidth, this
				.getFieldHeight()));
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createParallelGroup(
								...
								.addComponent(userLabel).addComponent(user))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(passwordLabel)
		...
		if (webserviceEnable) {
			...
			webserviceBorder = BorderFactory.createTitledBorder(Config.language
					.getProperty("Dialog.Settings.Webservice"));
			webservice.setBorder(webserviceBorder);

			// panel for url input
			JLabel urlLabel = new JLabel(
					Config.language.getProperty("Dialog.Settings.URL"));
			url = new JTextField(Config.config.getProperty("Program.URL"));
			url.setPreferredSize(new Dimension(fieldWidth, this
					.getFieldHeight()));
			...
			JLabel setLabel = new JLabel(
					Config.language.getProperty("Dialog.Settings.ProxySet"));
			proxySet = new JCheckBox();
			proxySet.setSelected(Boolean.parseBoolean(Config.config
					.getProperty("Proxy.Set")));

			webservice.add(setLabel);
			webservice.add(proxySet);

			JLabel hostLabel = new JLabel(
					Config.language.getProperty("Dialog.Settings.ProxyHost"));
			proxyHost = new JTextField(Config.config.getProperty("Proxy.Host"));
			proxyHost.setPreferredSize(new Dimension(fieldWidth, this
					.getFieldHeight()));
			...
			JLabel portLabel = new JLabel(
					Config.language.getProperty("Dialog.Settings.ProxyPort"));
			proxyPort = new JTextField(Config.config.getProperty("Proxy.Port"));
			proxyPort.setPreferredSize(new Dimension(fieldWidth, this
					.getFieldHeight()));
			...
			layout.setHorizontalGroup(layout
					.createSequentialGroup()
					.addGroup(
							layout.createParallelGroup().addComponent(urlLabel)
					...
					.addGroup(
							layout.createParallelGroup()
									.addComponent(url, 0,
											GroupLayout.DEFAULT_SIZE,
											Short.MAX_VALUE)
									.addComponent(proxySet, 0,
											GroupLayout.DEFAULT_SIZE,
											Short.MAX_VALUE)
									.addComponent(proxyHost, 0,
											GroupLayout.DEFAULT_SIZE,
											Short.MAX_VALUE)
									.addComponent(proxyPort, 0,
											GroupLayout.DEFAULT_SIZE,
											Short.MAX_VALUE)));
			layout.setVerticalGroup(layout
					.createSequentialGroup()
					.addGroup(
							layout.createParallelGroup(
									GroupLayout.Alignment.BASELINE)
									.addComponent(urlLabel).addComponent(url))
					.addGroup(
							layout.createParallelGroup(
									GroupLayout.Alignment.CENTER)
									.addComponent(setLabel)
									.addComponent(proxySet))
					.addGroup(
							layout.createParallelGroup(
									GroupLayout.Alignment.BASELINE)
									.addComponent(hostLabel)
		...
		BoxLayout boxlayout = new BoxLayout(notificationsPanel,
				BoxLayout.PAGE_AXIS);
		notificationsPanel.setLayout(boxlayout);
		TitledBorder notificationsBorder;
		notificationsBorder = BorderFactory.createTitledBorder(Config.language
				.getProperty("Dialog.Settings.Notifications"));
		notificationsPanel.setBorder(notificationsBorder);

		JLabel notificationsLabel = new JLabel(
				Config.language.getProperty("Dialog.Settings.NotificationsSet"));
		notificationsSet = new JCheckBox();
		notificationsSet.setSelected(Boolean.parseBoolean(Config.config
				.getProperty("Notifications.Set")));

		JLabel overwriteLabel = new JLabel(
				Config.language.getProperty("Dialog.Settings.OverwriteSet"));
		overwriteSet = new JCheckBox();
		overwriteSet.setSelected(Boolean.parseBoolean(Config.config
				.getProperty("Overwrite.Set")));

		JLabel filterOptionsLabel = new JLabel(
				Config.language.getProperty("Dialog.Settings.FilterOptionsSet"));
		String[] options = new String[] {
				Config.language.getProperty("Filter.Save.Changes.Keep"),
				Config.language.getProperty("Filter.Save.Changes.KeepApply"),
				Config.language.getProperty("Filter.Save.Changes.Discard") };
		filterCloseOptions = new JComboBox(options);
		filterCloseOptions.setSelectedIndex(Integer.parseInt(Config.config
				.getProperty("FilterClose.Option")));
		filterCloseSet = new JCheckBox();
		filterCloseSet.setSelected(Boolean.parseBoolean(Config.config
				.getProperty("FilterClose.Set")));
		...
		autosaveBorder = BorderFactory.createTitledBorder(Config.language
				.getProperty("Dialog.Settings.Autosave"));
		autosave.setBorder(autosaveBorder);

		// panel for enabling auto-save
		JLabel enableLabel = new JLabel(
				Config.language.getProperty("Dialog.Settings.AutosaveSet"));
		autosaveSet = new JCheckBox();
		if (Config.config.getProperty("Autosave.Set") != null
				&& Boolean.valueOf(Config.config.getProperty("Autosave.Set")) == true)
		...
		JLabel timerLabel = new JLabel(
				Config.language.getProperty("Dialog.Settings.AutosaveTimer"));
		...
		layout.setHorizontalGroup(layout
				.createSequentialGroup()
				.addGroup(
						layout.createParallelGroup().addComponent(enableLabel)
								.addComponent(timerLabel))
				.addPreferredGap(LayoutStyle.ComponentPlacement.RELATED,
						GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
				.addGroup(
						layout.createParallelGroup()
								.addComponent(autosaveSet, 0,
										GroupLayout.DEFAULT_SIZE,
										Short.MAX_VALUE)
								.addComponent(timerSpinner, 0,
										GroupLayout.DEFAULT_SIZE,
										Short.MAX_VALUE)));
		layout.setVerticalGroup(layout
				.createSequentialGroup()
		...
		if (webservice != null) {
			layout.setHorizontalGroup(layout.createSequentialGroup().addGroup(
					layout.createParallelGroup(GroupLayout.Alignment.LEADING)
							.addComponent(webservice, GroupLayout.DEFAULT_SIZE,
									GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
							.addComponent(notificationsPanel,
									GroupLayout.DEFAULT_SIZE,
									GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
							.addComponent(autosave, GroupLayout.DEFAULT_SIZE,
									GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
							.addComponent(login, GroupLayout.DEFAULT_SIZE,
									GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
			layout.setVerticalGroup(layout
					.createSequentialGroup()
					.addComponent(webservice)
					.addComponent(notificationsPanel)
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (cmd.equals("apply")) {
			...
			if (proxySet != null)
				Config.config.setProperty("Proxy.Set",
						Boolean.valueOf(proxySet.isSelected()).toString());
			if (proxyHost != null)
				Config.config.setProperty("Proxy.Host", proxyHost.getText());
			if (proxyPort != null)
				Config.config.setProperty("Proxy.Port", proxyPort.getText());
			Config.config.setProperty("Notifications.Set",
					Boolean.valueOf(notificationsSet.isSelected()).toString());
			Config.config.setProperty("Overwrite.Set",
					Boolean.valueOf(overwriteSet.isSelected()).toString());
			Config.config.setProperty("Autosave.Set",
					Boolean.valueOf(autosaveSet.isSelected()).toString());
			Config.config.setProperty("Autosave.Interval", timerSpinner
					.getValue().toString());
			Config.config.setProperty("FilterClose.Set",
					Boolean.valueOf(filterCloseSet.isSelected()).toString());
			Config.config.setProperty("FilterClose.Option",
					String.valueOf(filterCloseOptions.getSelectedIndex()));
