...
/**
 * this class extracts the data for the selected variable
 * from the graph according to the given filter criteria.
 *
...
public class DataExtractor implements Monitorable {
    ...
    /**
     * the set of Attribute filter pairs: a mapping from attribute names to values.
     */
    ...
    /**
     * the number of concepts that were relevent for the given
     * variable and the given filter criteria.
     */
    private int relevant_concepts;

    /**
     * the number of relations that were relevent for the given
     * variable and the given filter criteria.
     */
    private int relevant_relations;

    /**
     * the actual dataset. a collection of values for
     * the given variable. can consist of any number type.
     */
    ...
    /**
     * the minimal and maximal value found for
     * the dataset.
     */
     ...
     *
     * @param cc the concept class to filter for
     */
     ...
     *
     * @param cc the concept class to remove.
     ...
     *
     * @param rt the relation type to filter for.
     * @return if the operation was allowed.
     */
     ...
     *
     * @param rt the relation type to remove.
     */
     ...
     * the criterium consists of an attribute and a value.
     *
     ...
     *
     * @param an the attribute name that identfies the attribute
     *           filter criterium.
     */
            ...
            if (variable instanceof AttributeName) {
                ...
                if (Number.class.isAssignableFrom(an.getDataType())) {
                    ...
                    for (ONDEXConcept c : aog.getConcepts()) {
                        ...
                        if (c.getAttribute(an) != null) {
                            ...
                            dataset.add(val);
                            if (min_val == null
                                    || (val.doubleValue() < min_val.doubleValue()))
                                min_val = val;
                            if (max_val == null
                                    || (val.doubleValue() > max_val.doubleValue()))
                                max_val = val;
                        } else
                            continue;
                        relevant_concepts++;
                ...
                if (variable.equals("degree")) {
                    ...
                    for (ONDEXConcept c : aog.getConcepts()) {
                        ...
                        dataset.add(deg);
                        if (min_val == null
                                || (deg < min_val.intValue()))
                            min_val = deg;
                        if (max_val == null
                                || (deg > max_val.intValue()))
                            max_val = deg;
                        progress++;
                    }
                }
    ...
    /**
     * applies all filters to the given concept to find
     * out whether it is accepted or not
     *
     * @param c the concept.
    ...
    private boolean applyFilters(ONDEXConcept c) {
        ...
        while (it.hasNext()) {
            ...
            if (c.getAttribute(aog.getMetaData()
                    .getAttributeName(string)) != null) {
                if (c.getAttribute(
                        aog.getMetaData().getAttributeName(string))
    ...
    private boolean applyFilters(ONDEXRelation r) {
        ...
        while (it.hasNext()) {
            string = it.next();
            value = gdsFilters.get(string);
            if (r.getFromConcept().getAttribute(
                    aog.getMetaData().getAttributeName(string)) != null) {
                if (r.getFromConcept().getAttribute(
                        aog.getMetaData().getAttributeName(string))
                        .getValue().equals(value)) {
                    found_one = true;
    ...
    /**
     * @return the number of concepts relevant
     *         according to the filters and the variable.
     */
    ...
    /**
     * @return the number of relations relevant
     *         according to the filters and the variable.
     */
