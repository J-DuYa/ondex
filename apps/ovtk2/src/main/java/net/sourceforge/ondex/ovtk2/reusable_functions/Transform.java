...
public class Transform {
	...
	/**
	 * Modifies concept accessions by string replacement of a group based on a regex
	 * 
	 * e.g. modifyConceptAcessionsOnRegex("^\\d+(\\.\\d+){3}([^\\d]+)$", "EC", null, "EC", "", 2); //cleans up trailing junk on EC 4 digit accessions
	 * e.g. modifyConceptAcessionsOnRegex("^\\d+(\\.\\d+){2}(\\.-){1}([^\\d]+)$", "EC", null, "EC", "", 3); //cleans up trailing junk on EC 3 digit accessions
	 *
	 * @author hindlem
	 * 
	 * @param viewer the viewer containing the graph to work on
	 * @param regex the regex with as many groups as groupToReplace (group 0 is everything)
	 * @param ccName the name of the concept class of concepts to search in (can be null)
	 * @param concept_cvName the name of the cv of concepts to search in (can be null)
	 * @param accession_cvName the name of the cv of accessions to seach in (can not be null)
	 * @param replacementText teh text to replace the matching group
	 * @param groupToReplace the matching group to replace text (group 0 is everything)
	 * @return countReplacements number of concepts where replacements where made
	 */
	public static final int modifyConceptAcessionsOnRegex(
			OVTK2PropertiesAggregator viewer,
			String regex, 
			String ccName,
			String concept_cvName, 
		...
		for (ONDEXConcept concept : concepts) {
			...
			for(String oldAccession: replacements.keySet()) {
				...
				concept.deleteConceptAccession(oldAccession, 
