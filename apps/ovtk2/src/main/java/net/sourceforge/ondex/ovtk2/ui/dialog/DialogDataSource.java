...
public class DialogDataSource extends OVTK2Dialog {
	...
	public DialogDataSource(ONDEXGraph aog, DialogConcept conceptDialog) {
		...
		this.getContentPane().add(
				makeButtonsPanel("Dialog.DataSource.Apply",
						"Dialog.DataSource.Cancel"), BorderLayout.SOUTH);
		this.pack();
	}
	...
	public DialogDataSource(ONDEXGraph aog, DataSource dataSource) {
		...
		this.getContentPane().add(makeProperties(), BorderLayout.CENTER);
		this.getContentPane().add(
				makeButtonsPanel(null, "Dialog.DataSource.Cancel"),
				BorderLayout.SOUTH);
		this.pack();
	}
	...
	private JPanel makeProperties() {
		...
		TitledBorder propertiesBorder = BorderFactory
				.createTitledBorder(Config.language
						.getProperty("Dialog.DataSource.DataSource"));
		properties.setBorder(propertiesBorder);

		// DataSource id
		JLabel idLabel = new JLabel(
				Config.language.getProperty("Dialog.DataSource.ID"));
		...
		JLabel fullnameLabel = new JLabel(
				Config.language.getProperty("Dialog.DataSource.FullName"));
		...
		JLabel descriptionLabel = new JLabel(
				Config.language.getProperty("Dialog.DataSource.Description"));
	...
	private boolean validateEntry() {
		if (id.getText().trim().length() == 0 || id.getText().contains(" ")) {
			JOptionPane.showInternalMessageDialog(this, Config.language
					.getProperty("Dialog.DataSource.InvalidID"),
					Config.language
							.getProperty("Dialog.DataSource.InvalidTitle"),
					JOptionPane.ERROR_MESSAGE);
			return false;
		} else if (aog.getMetaData().checkDataSource(id.getText())) {
			JOptionPane.showInternalMessageDialog(this, Config.language
					.getProperty("Dialog.DataSource.DuplicateID"),
					Config.language
							.getProperty("Dialog.DataSource.DuplicateTitle"),
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (cmd.equals(APPLY)) {
			if (validateEntry()) {
				aog.getMetaData().createDataSource(id.getText(),
