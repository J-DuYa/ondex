    ...
    private InputStream getDescriptorXml(String zipFile, boolean transformXsl) throws TransformerException, IOException {
        ...
        while ((entry = zis.getNextEntry()) != null) {
            if (!entry.isDirectory()
                    && entry.getName().equals("workflow-component-description.xml") ||
                    (entry.getName().endsWith("workflow-component-description.xsl") && transformXsl)) {
                ...
                if (entry.getName().endsWith(".xml")) {
                    ...
                    transformer = tFactory.newTransformer(
                            new StreamSource(
                                    new ByteArrayInputStream(fout.toByteArray())));
                }
            }
        }
        zis.close();
