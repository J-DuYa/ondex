...
public class SavePlugin {
     ...
     *
     * @param artifact  maven artifact to download
     * @param pluginDir dir to download artifact to
     * @throws IOException error in downloading file
     */
    public static void downloadPlugin(Artifact artifact, File pluginDir) throws IOException {
        ...
        URL url = new URL(NexusURLs.NEXUS_SERVICE + "/local/artifact/maven/redirect?" + artifact.getNexusOptions().replaceAll("&", "&amp;")
                .replaceAll("workflow-component-description", "jar-with-dependencies").replaceAll("p=zip", "p=jar"));//TODO Dirty hack fix later
