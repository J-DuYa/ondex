...
public class DialogEvidenceType extends OVTK2Dialog {
	...
	public DialogEvidenceType(ONDEXGraph aog, OVTK2Dialog dialog) {
		...
		this.getContentPane().add(
				makeButtonsPanel("Dialog.EvidenceType.Apply", "Dialog.EvidenceType.Cancel"),
				BorderLayout.SOUTH);
		this.pack();
	}
	...
	public DialogEvidenceType(ONDEXGraph aog, EvidenceType et) {
		...
		this.getContentPane().setLayout(new BorderLayout());
		this.getContentPane().add(makeProperties(), BorderLayout.CENTER);
		this.getContentPane().add(makeButtonsPanel(null, "Dialog.EvidenceType.Cancel"),
				BorderLayout.SOUTH);
		this.pack();
	}
	...
	private JPanel makeProperties() {
		...
		TitledBorder propertiesBorder = BorderFactory
				.createTitledBorder(Config.language
						.getProperty("Dialog.EvidenceType.EvidenceType"));
		...
		JLabel fullnameLabel = new JLabel(
				Config.language.getProperty("Dialog.EvidenceType.FullName"));
		...
		JLabel descriptionLabel = new JLabel(
				Config.language.getProperty("Dialog.EvidenceType.Description"));
	...
	private boolean validateEntry() {
		if (id.getText().trim().length() == 0 || id.getText().contains(" ")) {
			JOptionPane.showInternalMessageDialog(this,
					Config.language.getProperty("Dialog.EvidenceType.InvalidID"),
					Config.language.getProperty("Dialog.EvidenceType.InvalidTitle"),
					JOptionPane.ERROR_MESSAGE);
			return false;
		} else if (aog.getMetaData().checkEvidenceType(id.getText())) {
			JOptionPane.showInternalMessageDialog(this,
					Config.language.getProperty("Dialog.EvidenceType.DuplicateID"),
					Config.language.getProperty("Dialog.EvidenceType.DuplicateTitle"),
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (cmd.equals(APPLY)) {
			if (validateEntry()) {
				aog.getMetaData().createEvidenceType(id.getText(),
