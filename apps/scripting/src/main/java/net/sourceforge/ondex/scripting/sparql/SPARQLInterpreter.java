...
/**
 * @author lysenkoa
 *         SPARQL parser. Maps the SPARQL commands to appropriate java functions in ONDEX/OVTK.
 *         
 *         TODO: Clarify: is this meant as a single instance object, or multiple console can be present at the same time ?
 */
public class SPARQLInterpreter implements CommandInterpreter {
	...
	/**
	 * Attempts to load the default configuration, a minimum set of commands required for correct
	...
	public boolean configure(){
		if(!isReady){
			...
			try{
				...
				if(querySet.getList("prefixes") == null) failed = true;
				if(querySet.getQuery("reified_type") == null) failed = true;
				if(querySet.getQuery("types_query") == null) failed = true;
				if(querySet.getQuery("reified_attribute_query") == null) failed = true;
				if(querySet.getQuery("reified_query") == null) failed = true;
				if(querySet.getQuery("concept_attribute_querry") == null) failed = true;
			}
			catch(Exception e){
				failed = true;
	...
	// these are now read from a file. The name of the set is the same as the .sqs file in the $ONDEX_DIR\config\query_sets directory
	// A URL which links a node representing a relation to its type which defines it as a relation.
	public String REIFIED_TYPE = "http://www.w3.org/1999/02/22-rdf-syntax-ns#Statement";
	
	// $$URI$$ is the URI of the entity representing the reified relation. The query must return a triple ?s ?p ?o representing the relation (?p predicate). This is limited, but this is what is supported now!	
	public String REIFIED_QUERY ="construct {?s ?p ?o} where { GRAPH ?g {" +
	...
	public String REIFIED_ATTRIBUTE_QUERY="select ?p ?o where { graph ?g {" +"<~~URI~~> ?p ?o . }}";
	public String TYPES_QUERY="select distinct ?x where { GRAPH ?g { <~~URI~~> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> ?x}} LIMIT 5";
	public String[] CLASSES_PREFRENCE_LIST=null;
	public String CONCEPT_ATTRIBUTES_QUERY="select ?p ?o where {graph ?g {<~~URI~~> ?p ?o}} limit 20";
	*/


	/**
	 * Main command parsing "loop". Plus function related to the SPARQL console operations.
	 */
	...
	private class ConnectHandler implements CommandHandler {
		public final Pattern loadPath = Pattern.compile("connect\\s*?\\(\\\"(.+?)\\\"\\)", Pattern.CASE_INSENSITIVE);
			...
			if (m.find()) {
				...
				if (match != null) {
					return new Callable() {
						...
						{
							myEndPoint = match;
							HttpURLConnection connection = (HttpURLConnection) new URL(match).openConnection();
	...
	/**
	 * Check for a list of types associated to a resource
	 * TODO the LIMIT in query could be made parametric depending on an optimization parameter.
	 */
	...
	/**
	 * Expand a reified statement (add nodes, and relations). If the expansion doesn't succeed, it returns a null relation
	 * @param uri
	 * @return
	...
	private ONDEXRelation expandReifiedStatement(String uri) throws InvocationTargetException, IllegalAccessException {
		...
		if(!p.isURI()) return r;	// If the property is not an URI the relation expansion has failed.
		if(!s.isURI()) return r;	// If the subject is not an URI the relation expansion has failed.
		if(!o.isURI()) return r;	// If the object is not an URI the relation expansion has failed.
